...
def _standardize_dataset(dataframe, means, stdevs):
    for row in dataframe:
        for i in range(len(row)):
            row[i] = (row[i] - means[i]) / stdevs[i]


def _get_dataset_minmax(dataframe):
...
def _minmax_dataset(dataframe, minmax):
    ...
    self : object
        Scaled dataset
    """
    minmax = _get_dataset_minmax(dataframe)
    for row in dataframe:
        for i in range(len(row)):
